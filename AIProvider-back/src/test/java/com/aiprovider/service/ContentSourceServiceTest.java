...
class ContentSourceServiceTest {
    @Test void createsPublicWebSourceFromHandleWithoutUidOrCredential(){
        ContentOperationsRepository repository=mock(ContentOperationsRepository.class);ContentSourceCreateDTO dto=new ContentSourceCreateDTO();dto.setPlatform("TWITTER");dto.setAdapterType("TWITTER_WEB");dto.setExternalHandle("@elonmusk");dto.setName("Elon Musk");
        ...
        assertEquals("elonmusk",new ContentSourceService(repository,mock(ContentPlatformSecretCipher.class),mock(TwitterTimelineClient.class),mock(TwitterPublicWebClient.class),new ObjectMapper()).create(dto).getExternalHandle());
        verify(repository).insertSource(argThat((ContentOperationsMapper.SourceRecord row)->row.getExternalUid()==null&&"elonmusk".equals(row.getExternalHandle())&&"https://x.com/elonmusk".equals(row.getSourceUrl())));
    }
    ...
    @Test void fetchesPublicWebSourceWithoutCredential(){ContentOperationsRepository repository=mock(ContentOperationsRepository.class);TwitterPublicWebClient web=mock(TwitterPublicWebClient.class);ObjectMapper json=new ObjectMapper();Map<String,Object> source=new HashMap<>();source.put("id",8L);source.put("platform","TWITTER");source.put("adapterType","TWITTER_WEB");source.put("externalHandle","elonmusk");source.put("name","Elon Musk");when(repository.findSource(8)).thenReturn(source);when(web.fetchLatest("elonmusk")).thenReturn(new TwitterFetchedPost("3","public latest","https://x.com/elonmusk/status/3",LocalDateTime.now(),json.createObjectNode()));when(repository.insertContentItem(any())).thenReturn(1);when(repository.findContentItems(8,1)).thenReturn(Collections.emptyList());assertEquals(1,new ContentSourceService(repository,mock(ContentPlatformSecretCipher.class),mock(TwitterTimelineClient.class),web,json).testFetch(8).getNewCount());verify(web).fetchLatest("elonmusk");}
}
