...
class ContentAiConfigServiceTest {
    ...
    private static Map<String,Object> config(){Map<String,Object> m=new HashMap<>();m.put("enabled",true);m.put("apiBaseUrl","https://generativelanguage.googleapis.com");m.put("model","gemini-3.5-flash");m.put("apiKeyEncrypted","encrypted-value");m.put("apiKeyHint","••••1234");m.put("relevancePrompt","这是一个长度足够的人工智能内容相关性判断提示词，用于单元测试验证。");m.put("contentRewritePrompt","这是一个长度足够的内容改写系统提示词，用于单元测试验证。");m.put("commentReplyPrompt","这是一个长度足够的评论回复系统提示词，用于单元测试验证。");m.put("temperature",new BigDecimal("0.7"));m.put("maxOutputTokens",2048);return m;}
}
