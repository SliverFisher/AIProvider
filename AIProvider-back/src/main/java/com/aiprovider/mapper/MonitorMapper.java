...
public interface MonitorMapper {
    @Select("SELECT COUNT(*) totalRequests, " +
        ...
        "FROM LlmCallLogs WHERE CreatedAt >= #{since}")
    ...
    @Select("WITH ranked AS (SELECT DurationMs, ROW_NUMBER() OVER (ORDER BY DurationMs) rn, COUNT(*) OVER () total " +
        "FROM LlmCallLogs WHERE CreatedAt >= #{since} AND CompletedAt IS NOT NULL) " +
    ...
    @Select("<script>SELECT " +
        ...
        "FROM LlmCallLogs WHERE CreatedAt >= DATE_SUB(NOW(), INTERVAL #{hours} HOUR) GROUP BY bucket ORDER BY bucket</script>")
    ...
    @Select("WITH base AS (SELECT " +
        ...
        "FROM LlmCallLogs WHERE CreatedAt >= DATE_SUB(NOW(), INTERVAL #{hours} HOUR) AND CompletedAt IS NOT NULL), " +
    ...
    @Select("SELECT Provider provider, Model model, COUNT(*) callCount, " +
        ...
        "FROM LlmCallLogs WHERE CreatedAt >= DATE_SUB(NOW(), INTERVAL 30 DAY) GROUP BY Provider,Model ORDER BY callCount DESC")
    List<Map<String,Object>> providerActivity();

    @Select("SELECT DefaultProvider defaultProvider, SelectedProvider selectedProvider, LocalQwenModel localQwenModel, GeminiModel geminiModel, UpdatedAt updatedAt FROM LlmProviderSelections ORDER BY UpdatedAt DESC LIMIT 1")
    ...
    @Select("<script>SELECT Id id, CreatedAt occurredAt, Source requestType, Provider provider, Model model, ResponseStatusCode statusCode, DurationMs durationMs, LEFT(Error,1000) error " +
        "FROM LlmCallLogs WHERE CreatedAt &gt;= DATE_SUB(NOW(), INTERVAL #{hours} HOUR) AND NOT (ResponseStatusCode BETWEEN 200 AND 399 AND COALESCE(Error,'')='') " +
    ...
    @Select("<script>SELECT COUNT(*) FROM LlmCallLogs WHERE CreatedAt &gt;= DATE_SUB(NOW(), INTERVAL #{hours} HOUR) AND NOT (ResponseStatusCode BETWEEN 200 AND 399 AND COALESCE(Error,'')='') " +
    ...
    @Delete("DELETE FROM LlmCallLogs WHERE CreatedAt &lt; DATE_SUB(NOW(), INTERVAL #{days} DAY)")
