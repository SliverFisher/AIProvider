...
public interface TwitterMapper {
    @Select("SELECT Id id, Username username, SessionStatus sessionStatus, LastLoginAt lastLoginAt, LastError lastError FROM TwitterAccounts ORDER BY UpdatedAt DESC")
    List<Map<String, Object>> findAccounts();

    @Select("SELECT Id id, Username username, EncryptedStorageState encryptedStorageState, SessionStatus sessionStatus, LastLoginAt lastLoginAt, LastError lastError FROM TwitterAccounts WHERE Id=#{id}")
    Map<String, Object> findAccount(@Param("id") long id);

    @Insert("INSERT INTO TwitterAccounts(Username, EncryptedStorageState, SessionStatus, LastLoginAt, LastError) " +
    ...
    @Select("SELECT Id FROM TwitterAccounts WHERE Username=#{username}")
    Long findAccountIdByUsername(@Param("username") String username);

    @Insert("INSERT INTO TwitterAccounts(Username, SessionStatus, LastLoginAt, LastError) " +
    ...
    @Update("UPDATE TwitterAccounts SET SessionStatus=#{status}, LastError=#{error} WHERE Id=#{id}")
    int updateAccountStatus(@Param("id") long id, @Param("status") String status, @Param("error") String error);

    @Insert("INSERT INTO TwitterPosts(AccountId, Content, Status, ScheduledAt, Source) VALUES(#{accountId}, #{content}, 'PENDING', #{scheduledAt}, #{source})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    void insertPost(PostInsert post);

    @Insert("INSERT INTO TwitterPostMedia(PostId, AssetId, StoragePath, LocalPath, LocalSource, OriginalFileName, ContentType, FileSize, Sha256, SortOrder) " +
    ...
    @Select("SELECT p.Id id, p.AccountId accountId, a.Username username, p.Content content, p.Status status, " +
            ...
            "FROM TwitterPosts p JOIN TwitterAccounts a ON a.Id=p.AccountId WHERE p.Id=#{id}")
    ...
    @Select("SELECT Id id, PostId postId, AssetId assetId, StoragePath storagePath, LocalPath localPath, LocalSource localSource, OriginalFileName originalFileName, ContentType contentType, " +
            "FileSize fileSize, Sha256 sha256, SortOrder sortOrder FROM TwitterPostMedia WHERE PostId=#{postId} ORDER BY SortOrder")
    List<Map<String, Object>> findMedia(@Param("postId") long postId);

    @Select("SELECT Id id, PostId postId, AssetId assetId, StoragePath storagePath, LocalPath localPath, LocalSource localSource, OriginalFileName originalFileName, ContentType contentType, " +
            "FileSize fileSize, Sha256 sha256, SortOrder sortOrder FROM TwitterPostMedia WHERE PostId=#{postId} AND Id=#{id}")
    Map<String, Object> findMediaItem(@Param("postId") long postId, @Param("id") long id);

    @Select("SELECT p.Id id, p.AccountId accountId, a.Username username, p.Content content, p.Status status, " +
            "p.TweetUrl tweetUrl, p.ErrorMessage errorMessage, p.AttemptCount attemptCount, p.ScheduledAt scheduledAt, p.Source source, p.SentAt sentAt, p.CreatedAt createdAt " +
            "FROM TwitterPosts p JOIN TwitterAccounts a ON a.Id=p.AccountId ORDER BY p.CreatedAt DESC LIMIT #{limit}")
    List<Map<String, Object>> findPosts(@Param("limit") int limit);

    @Select("SELECT p.Id id, p.AccountId accountId, a.Username username, p.Content content, p.Status status, " +
            "p.TweetUrl tweetUrl, p.ErrorMessage errorMessage, p.AttemptCount attemptCount, p.ScheduledAt scheduledAt, p.Source source, p.SentAt sentAt, p.CreatedAt createdAt " +
            "FROM TwitterPosts p JOIN TwitterAccounts a ON a.Id=p.AccountId " +
    ...
    @Update("UPDATE TwitterPosts SET Status='PROCESSING', ErrorMessage=NULL, AttemptCount=AttemptCount+1 " +
            "WHERE Id=#{id} AND Status='PENDING' AND ScheduledAt <= NOW(3)")
    int claimPost(@Param("id") long id);

    @Update("UPDATE TwitterPosts SET Status='SENT', TweetUrl=#{tweetUrl}, ErrorMessage=NULL, SentAt=NOW(3) WHERE Id=#{id} AND Status='PROCESSING'")
    int markPostSent(@Param("id") long id, @Param("tweetUrl") String tweetUrl);

    @Update("UPDATE TwitterPosts SET Status='FAILED', ErrorMessage=#{error} WHERE Id=#{id} AND Status='PROCESSING'")
    int markPostFailed(@Param("id") long id, @Param("error") String error);

    @Update("UPDATE TwitterPosts SET Status='PENDING', ErrorMessage=NULL WHERE Id=#{id} AND Status='FAILED'")
    int retryPost(@Param("id") long id);

    @Update("UPDATE TwitterPosts SET Status='CANCELLED', ErrorMessage=NULL WHERE Id=#{id} AND Status IN ('PENDING','FAILED')")
    int cancelPost(@Param("id") long id);

    @Update("UPDATE TwitterPosts SET Status='PENDING', ErrorMessage='服务重启，任务已重新排队' WHERE Status='PROCESSING'")
    int recoverProcessingPosts();

    @Update("UPDATE TwitterPosts SET Status='PENDING', ErrorMessage='本机发布租约超时，任务已重新排队' " +
            "WHERE Status='PROCESSING' AND UpdatedAt < DATE_SUB(NOW(3), INTERVAL 10 MINUTE)")
    int recoverStaleClientPosts();

    @Select("SELECT Id FROM TwitterPosts WHERE Status='PENDING' AND ScheduledAt <= NOW(3) ORDER BY ScheduledAt, CreatedAt")
