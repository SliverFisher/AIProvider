...
public interface LocalGeneratedImageMapper {
    @Insert({"<script>",
            "INSERT INTO c_LocalGeneratedImages(Platform,PathHash,PromptId,ImagePath,FileName,MediaType,MimeType,WorkflowId,WorkflowName,PromptSchemeName,PromptMode,Prompt,NegativePrompt,MainModel,LorasJson,Seed,Steps,Cfg,Sampler,Scheduler,Width,Height,TaskCreatedAt,GenerationCompletedAt,GenerationDurationMs) VALUES",
            "<foreach collection='rows' item='row' separator=','>(#{platform},#{row.pathHash},#{row.item.promptId},#{row.item.imagePath},#{row.item.fileName},#{row.item.mediaType},#{row.item.mimeType},#{row.item.workflowId},#{row.item.workflowName},#{row.item.promptSchemeName},#{row.item.promptMode},#{row.item.prompt},#{row.item.negativePrompt},#{row.item.mainModel},#{row.item.lorasJson},#{row.item.seed},#{row.item.steps},#{row.item.cfg},#{row.item.sampler},#{row.item.scheduler},#{row.item.width},#{row.item.height},#{row.item.taskCreatedAt},#{row.item.generationCompletedAt},#{row.item.generationDurationMs})</foreach>",
            "ON DUPLICATE KEY UPDATE PromptId=VALUES(PromptId),FileName=VALUES(FileName),MediaType=VALUES(MediaType),MimeType=VALUES(MimeType),Status='ACTIVE',WorkflowId=VALUES(WorkflowId),WorkflowName=VALUES(WorkflowName),PromptSchemeName=VALUES(PromptSchemeName),PromptMode=VALUES(PromptMode),Prompt=VALUES(Prompt),NegativePrompt=VALUES(NegativePrompt),MainModel=VALUES(MainModel),LorasJson=VALUES(LorasJson),Seed=VALUES(Seed),Steps=VALUES(Steps),Cfg=VALUES(Cfg),Sampler=VALUES(Sampler),Scheduler=VALUES(Scheduler),Width=VALUES(Width),Height=VALUES(Height),TaskCreatedAt=VALUES(TaskCreatedAt),GenerationCompletedAt=VALUES(GenerationCompletedAt),GenerationDurationMs=VALUES(GenerationDurationMs),UpdatedAt=CURRENT_TIMESTAMP(3)",
            "</script>"})
    int upsertBatch(@Param("platform") String platform, @Param("rows") List<Map<String,Object>> rows);

    @Select("SELECT Id id,Platform platform,PathHash pathHash,PromptId promptId,ImagePath imagePath,FileName fileName,MediaType mediaType,MimeType mimeType,Status status,WorkflowId workflowId,WorkflowName workflowName,PromptSchemeName promptSchemeName,PromptMode promptMode,Prompt prompt,NegativePrompt negativePrompt,MainModel mainModel,LorasJson lorasJson,Seed seed,Steps steps,Cfg cfg,Sampler sampler,Scheduler scheduler,Width width,Height height,TaskCreatedAt taskCreatedAt,GenerationCompletedAt generationCompletedAt,GenerationDurationMs generationDurationMs,CreatedAt createdAt,UpdatedAt updatedAt " +
