...
public interface LocalGeneratedImageMapper {
    @Insert({"<script>",
            "INSERT INTO c_LocalGeneratedImages(Platform,PathHash,PromptId,ImagePath,FileName,MediaType,MimeType,WorkflowId,WorkflowName,Prompt,NegativePrompt,MainModel,LorasJson,Seed,Steps,Cfg,Sampler,Scheduler,Width,Height,TaskCreatedAt,GenerationCompletedAt,GenerationDurationMs) VALUES",
            "<foreach collection='rows' item='row' separator=','>(#{platform},#{row.pathHash},#{row.item.promptId},#{row.item.imagePath},#{row.item.fileName},#{row.item.mediaType},#{row.item.mimeType},#{row.item.workflowId},#{row.item.workflowName},#{row.item.prompt},#{row.item.negativePrompt},#{row.item.mainModel},#{row.item.lorasJson},#{row.item.seed},#{row.item.steps},#{row.item.cfg},#{row.item.sampler},#{row.item.scheduler},#{row.item.width},#{row.item.height},#{row.item.taskCreatedAt},#{row.item.generationCompletedAt},#{row.item.generationDurationMs})</foreach>",
