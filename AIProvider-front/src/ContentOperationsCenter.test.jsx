...
describe("ContentOperationsCenter", () => {
  ...
  it("loads Gemini settings without exposing the existing API key", async()=>{
    ...
    expect(await screen.findByText("Gemini 内容生成")).toBeTruthy();
    expect(screen.getByRole("link",{name:"前往账号中心"}).getAttribute("href")).toBe("/accounts");
  ...
  it("keeps all credential entry points in account center",async()=>{vi.stubGlobal("fetch",vi.fn(async()=>ok(overview)));render(<ContentOperationsCenter/>);fireEvent.click(await screen.findByRole("button",{name:"采集源"}));expect(screen.getByRole("link",{name:"前往账号中心"}).getAttribute("href")).toBe("/accounts");expect(screen.queryByRole("button",{name:"添加采集账号"})).toBeNull();expect(screen.queryByLabelText("X Cookie")).toBeNull();expect(screen.queryByLabelText("Bearer Token")).toBeNull();});
